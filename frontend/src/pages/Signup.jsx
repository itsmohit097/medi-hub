        ...
        <div className="w-full max-w-md">
          ...
          <form className="flex flex-col" onSubmit={handleSignup}>
            ...
            <div className="m-2 flex flex-col-reverse md:flex-row-reverse items-center">
              <div className="w-1/2">
                <div style={{ height: '10px', width: `${strength * 25}%`, backgroundColor: strength === 1 ? 'red' : strength === 2 ? 'orange' : strength === 3 ? 'yellow' : 'green' }}></div>
              </div>
              <div className="w-1/2 flex items-center justify-around mt-4 md:mt-0">
                {['Weak', 'Fair', 'Good', 'Strong'].map((label, index) => (
                  <div key={index} className={`text-sm ${index < strength ? 'font-bold' : ''}`}>{label}</div>
                ))}
              </div>
            </div>
            <button className="bg-main_theme text-white font-bold py-2 px-4 rounded-md mt-4">
                    ...
                    <Link to="/login">
                        <p className="my-3 p-1 md:p-0 text-purple-600 hover:underline"  style={{ color: 'rgb(27, 120, 120)' }}>
