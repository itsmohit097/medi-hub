...
const AddtoCart = () => {
  ...
  const GetUsercartitems = () => {
    ...
    CartIds.forEach((element) => {
      getMedicines(element.medicineId).then(
        (res) => {
          setCartItems((prev) => {
            return [
              ...prev,
  ...
  const removeItem = (itemId) => {
    deleteFromUserCart({
      ...
      .catch((err) => {
        console.log(
          "the error in removing cart addtocart page",
          err
        );
      });
  };
  ...
  const handlePaymentVerify = async (data) => {
    const options = {
      ...
      name: "medihub",
      description:
        "Your buying something important for your health",
      order_id: data.id,
    ...
    <div className="container mx-auto my-10 relative  p-6 bg-white rounded-lg shadow-md flex gap-4">
      ...
      <div className="divide-y divide-gray-500 w-8/12 ">
        ...
        {cartItems.map((item) => (
          <div
            key={item.id}
            className="flex items-center border-2 m-2 py-4 rounded-lg">
            <img
              src={item.image}
              alt={item.name}
