...
function MediHubBot() {
  ...
  return (
    <div className={`flex flex-col fixed bottom-24 z-50 left-10 h-[70vh] w-[18.5rem] p-4 shadow-md shadow-emerald-200 z-40 bg-white rounded-t-lg rounded-r-lg`}>
    ...
    <div className={`flex-1 overflow-y-auto bg-gray-100 rounded-md p-2`}>
       {messages.map((msg,index)=>(
         ...
         >
           <div
            className={` px-2 text-xs ${msg.role === "user" ? `text-white ml-32 p-1 rounded-t-lg rounded-l-lg bg-dark_theme` : ` bg-light_theme text-gray-800 rounded-b-lg rounded-r-lg`}`}
           >
           {msg.text}
           </div>
